-- Migration: Create job_photos table for storing before/after photos
...
BEGIN 
    ...
    IF NOT EXISTS (
        ...
        WHERE table_name = 'job_photos' AND column_name = 'assignment_id'
    ) THEN
        ALTER TABLE job_photos ADD COLUMN assignment_id UUID REFERENCES job_assignments(id) ON DELETE CASCADE;
    END IF;
    
    -- Add uploaded_by if missing
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'job_photos' AND column_name = 'uploaded_by'
    ) THEN
        ALTER TABLE job_photos ADD COLUMN uploaded_by UUID REFERENCES auth.users(id);
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE job_photos ADD COLUMN uploaded_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
    
    -- Add description if missing
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'job_photos' AND column_name = 'description'
    ) THEN
        ALTER TABLE job_photos ADD COLUMN description TEXT;
    END IF;
    
    -- Add created_at if missing
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'job_photos' AND column_name = 'created_at'
    ) THEN
        ALTER TABLE job_photos ADD COLUMN created_at TIMESTAMPTZ DEFAULT NOW();
    END IF;
END $$;

-- Create indexes
CREATE INDEX IF NOT EXISTS idx_job_photos_job_id ON job_photos(job_id);
CREATE INDEX IF NOT EXISTS idx_job_photos_assignment_id ON job_photos(assignment_id);
...
DROP POLICY IF EXISTS "Workers can view their job photos" ON job_photos;
DROP POLICY IF EXISTS "Business owners can view job photos" ON job_photos;
DROP POLICY IF EXISTS "Workers can upload photos for their jobs" ON job_photos;
DROP POLICY IF EXISTS "Business owners can upload job photos" ON job_photos;
DROP POLICY IF EXISTS "Workers can delete their photos" ON job_photos;
DROP POLICY IF EXISTS "Business owners can delete job photos" ON job_photos;
